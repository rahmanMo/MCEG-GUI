...
# Getting csv automatically and inserting into mongodb for MVT STG3 via Adhoc processor
def STG3_CSV_TO_MONGO():
    ...
    txtFileNamePatterns = ['MCEG_STG3D0*.txt',
                           'MCEG_STG3D1*.txt',
                           'MCEG_STG3D2*.txt',
                           'MCEG_STG3D3*.txt',
                           'MCEG_STG3D4*.txt',
                           ...
                           'MCEG_STG3D6*.txt',
                           'MCEG_STG3D7*.txt']
    csvFileNamePatterns = ['MCEG_STG3D0*.csv',
                           'MCEG_STG3D1*.csv',
                           'MCEG_STG3D2*.csv',
                           'MCEG_STG3D3*.csv',
                           'MCEG_STG3D4*.csv',
                           ...
                           'MCEG_STG3D6*.csv',
                           'MCEG_STG3D7*.csv']
    allTxtFileData = [{'fileName': f'MCEG_STG3D0_ADH004_{timeStamp}', 'dateString': f'ADH004_{d0}', 'day': 'D0'},
                      {'fileName': f'MCEG_STG3D1_ADH004_{timeStamp}',
                          'dateString': f'ADH004_{d1}', 'day': 'D1'},
                      {'fileName': f'MCEG_STG3D2_ADH004_{timeStamp}',
                          'dateString': f'ADH004_{d2}', 'day': 'D2'},
                      {'fileName': f'MCEG_STG3D3_ADH004_{timeStamp}',
                          'dateString': f'ADH004_{d3}', 'day': 'D3'},
                      {'fileName': f'MCEG_STG3D4_ADH004_{timeStamp}',
                          'dateString': f'ADH004_{d4}', 'day': 'D4'},
                      {'fileName': f'MCEG_STG3D5_ADH004_{timeStamp}',
                          'dateString': f'ADH004_{d5}', 'day': 'D5'},
                      {'fileName': f'MCEG_STG3D6_ADH004_{timeStamp}',
                          'dateString': f'ADH004_{d6}', 'day': 'D6'},
                      {'fileName': f'MCEG_STG3D7_ADH004_{timeStamp}', 'dateString': f'ADH004_{d7}', 'day': 'D7'}]
    ...
    for txtFileData in allTxtFileData:
        ...
        if fnmatch.filter(receivedFiles, fileName):
            ...
            shutil.copy(
                f'{stageReceive}/{foundFile[0]}', f"{currentDirectory}/STG3/STG3{day}.csv")
    ...
    for txtFileData in allTxtFileData:
        ...
        if fnmatch.filter(localFiles, fileName):
            ...
            jsonFileName = f"STG3{day}.json"
            #  Making csv and json version of the file
            with open(f'{currentDirectory}/STG3/{foundFile[0]}', newline='') as f:
                r = csv.reader(f)
                data = [line for line in r]
            with open(f'{currentDirectory}/STG3/{foundFile[0]}', 'w', newline='') as f:
                w = csv.writer(f)
                w.writerow(['recordStatus',
                            ...
                            'CTFlightNumber'])
                w.writerows(data)
            with open(f'{currentDirectory}/STG3/{foundFile[0]}') as f:
                reader = csv.DictReader(f)
                rows = list(reader)

            with open(f'{currentDirectory}/STG3/{jsonFileName}', 'w') as f:
                json.dump(rows, f)
    ...
    for txtFileData in allTxtFileData:
        ...
        if fnmatch.filter(localFiles, fileName):
            ...
            try:
                ...
                collection_name = f'STG3{day}'
                db_cm = mng_db[collection_name]
                cdir = Path(os.getcwd())
                file_res = f'{cdir}/STG3/STG3{day}.csv'
